...
fn lumina_bin() -> std::path::PathBuf {
    let path = std::path::PathBuf::from(env!("CARGO_BIN_EXE_lumina"));
    path
}
...
fn test_fleet_lum_runs_without_error() {
    ...
    assert!(output.status.success(), "lumina run failed:\n{stderr}");

    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        stdout.contains("Moto"),
        "expected Moto in output:\n{stdout}"
    );
    assert!(
        stdout.contains("battery"),
        "expected battery in output:\n{stdout}"
    );
}
...
fn test_on_clear_fires_when_condition_recovers() {
    ...
    std::fs::write(
        ...
        concat!(
            ...
            "} on clear {\n",
            "  show \"RESOLVED: temperature normal\"\n",
            "}\n",
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        stdout.contains("ALERT: overheating"),
        "expected alert message in:\n{stdout}"
    );
    assert!(
        stdout.contains("RESOLVED: temperature normal"),
