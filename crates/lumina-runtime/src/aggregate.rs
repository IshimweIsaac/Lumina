                    ...
                    AggregateScope::Cluster | AggregateScope::Region(_) => {
                        if let Some(cs) = cluster_state {
                            if let Some(f) = source_field {
                                cs.values()
                                    .filter_map(|node_state| node_state.get(f).cloned())
                                    .collect()
                            } else {
                                // Count(*) across nodes
                            ...
                            if let Some(f) = source_field {
                                if let Some(idx) = entity_schema.field_indices.get(f) {
                                    store
                                        ...
                                        .filter_map(|(_, inst)| inst.get(*idx).cloned())
                                        .collect()
                                } else {
                                    vec![]
                                }
...
fn compute_agg_from_values(expr: &AggregateExpr, values: Vec<Value>) -> Value {
    match expr {
        ...
        AggregateExpr::Count(None) => Value::Number(values.len() as f64),
        AggregateExpr::Count(Some(_)) => {
            let bs: Vec<bool> = values
                .into_iter()
                .filter_map(|v| {
