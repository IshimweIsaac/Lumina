...
fn test_multi_instance_adapter_polling() {
    ...
    schema.register_field(
        "Sensor",
        "temp",
        &lumina_analyzer::types::LuminaType::Number,
    );
    ...
    ev.instances
        .insert("sensor-A".to_string(), "Sensor".to_string());
    ev.instances
        .insert("sensor-B".to_string(), "Sensor".to_string());
    ...
    assert_eq!(
        ev.get_instance_field(inst_a, "temp").unwrap(),
        Value::Number(30.0)
    );

    let inst_b = ev.store.get("sensor-B").expect("sensor-B missing");
    assert_eq!(
        ev.get_instance_field(inst_b, "temp").unwrap(),
        Value::Number(35.0)
    );
}
