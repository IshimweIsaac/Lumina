...
async fn test_cross_node_state_sync() {
    ...
    ev1.apply_update("s1", "temp", lumina_runtime::value::Value::Number(50.0))
        .unwrap();
    ...
    assert!(
        ev2_cluster_view.contains_key("node1"),
        "Node 2 should have state for node1"
    );
    let node1_state = ev2_cluster_view.get("node1").unwrap();
    assert_eq!(
        node1_state.get("temp"),
        Some(&lumina_runtime::value::Value::Number(50.0))
    );
