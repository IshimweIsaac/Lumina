...
|---------------------------------------------|---------------------------------------------------------------|
| Understand what Lumina IS and how to think  | [01-mental-model.md](#mental-model)           |
| Look up syntax for ANY construct       | [02-syntax-reference.md](#syntax-reference)       |
| Understand types, operators, coercion    | [03-type-system.md](#type-system)            |
| Find a built-in function signature      | [04-built-in-functions.md](#built-in-functions)     |
| Write or debug a reactive rule        | [05-rules-and-triggers.md](#rules-and-triggers)     |
| See complete, runnable example programs   | [06-patterns-cookbook.md](#patterns-cookbook)       |
| Diagnose an error code (L-codes / R-codes)  | [07-error-encyclopedia.md](#error-encyclopedia)     |
| Use clusters, FFI, external entities, secrets| [08-advanced-features.md](#advanced-features)      |
| Check if something is supported or broken  | [09-known-limitations.md](#known-limitations)      |
| Architect a full project from scratch    | [10-project-templates.md](#project-templates)      |
| Install, update, or use the CLI       | [11-cli-reference.md](#cli-reference)          |
...
See [11-cli-reference.md](#cli-reference) for the full command reference.


---

<!-- Source: 01-mental-model.md -->
...
when MainServer.is_unhealthy becomes true {
  migrate("critical_db", to: "backup_node")
}
```
...
<!-- Source: 02-syntax-reference.md -->

# Lumina Syntax Reference
...
### [v2.1] Resource Entity (Infrastructure as Code)

Declares desired infrastructure state bound to a specific provider adapter.
...
alert
  ...
  code: 404,
  runbook: "https://wiki/runbook-404"
```

Parameters: `severity` (required), `message` (required), `source` (optional), `code` (optional), plus any arbitrary custom key-value pairs (e.g., `runbook`).
...
Emits debugging trace information during rule evaluation.

```lumina
trace "Evaluating threshold: {max_temp}"
```

### for — Iterating over Entities in Actions

Executes a block of actions for every instance of an entity type.

```lumina
for (s: Server) {
  update s.is_quarantined = true
}
```

### [v2.1] provision / reconcile / destroy

Infrastructure orchestration actions for `resource entity` instances.

```lumina
provision my_docker_app
reconcile my_docker_app
destroy my_docker_app
```
...
### [v2.1] Cluster State Access

Access state across the distributed network natively.

```lumina
cluster.my_node_2.cpu_temp  -- Read a field from a specific node
cluster.all.is_online     -- Returns a list of values from all nodes
```
...
<!-- Source: 04-built-in-functions.md -->
...
<!-- Source: 05-rules-and-triggers.md -->
...
<!-- Source: 06-patterns-cookbook.md -->
...
<!-- Source: 08-advanced-features.md -->
...
-- Deploy (simplified in Lumina — spec is evaluated, leader broadcasts)
deploy("deployment-spec")
```
...
<!-- Source: 09-known-limitations.md -->

# Lumina Known Limitations
...
The EBNF spec (`SPEC.md`) still mentions `update path 'to' expr`. the Lumina standard uses `=`:
...
These features appear in the old `master_knowledge.md` or pattern descriptions but are **NOT implemented** in the Lumina runtime:
...
| eBPF integration | ❌ Roadmap | Not available |
| Natural Language compiler | ❌ Roadmap | Not available |
| Edge Federation | ❌ Roadmap | Not available |
...
<!-- Source: 10-project-templates.md -->
...
See [08-advanced-features.md](#advanced-features) for cluster configuration details.
...
```bash
lumina --version
# Output: Lumina: The Cluster Release
```
